...
- Issue 1: Fixed. Desktop run-panel drawer remains available.
- Issue 2: Fixed. Socket-close cleanup no longer disposes the socket-state watcher.
- Issue 3: Partially addressed. Client-store hydration guards are present, but there is no account-fetch/init method in this checkout to add meaningful request caching to.
- Issue 4: Fixed. Trade-type option loading is triggered after the initial Blockly workspace XML is hydrated.
- Issue 5: Blocked. The referenced Copy Trading page and manager do not exist in this checkout, so there is no implementation to repair.
- Issue 6: Fixed in the client path. DTrader bridge metadata now uses the configured Deriv app ID, including the Kenya fallback (`121856`). Server-side app registration and availability remain deployment responsibilities.
