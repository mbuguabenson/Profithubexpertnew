...
const MultiTrader: React.FC = observer(() => {
    ...
    const resolvers = useRef<
        Map<number, { resolve: (d: any) => void; reject: (e: any) => void; isSubscription?: boolean }>
    >(new Map());
            ...
            try {
                ...
                if (data.msg_type === 'authorize') {
                    if (data.error) {
                        ...
                        setStatus('connected');
                        addLog(
                            `Authorized as ${data.authorize.loginid} (${data.authorize.currency || 'USD'})`,
                            'success'
                        );
    ...
    const connect = useCallback(async (): Promise<boolean> => {
        ...
        if (
            ...
            (api_base.is_authorized || client?.is_logged_in)
        ) {
            setStatus('connected');
        ...
        const currentToken =
            (typeof client?.getToken === 'function' ? client.getToken() : null) ||
    ...
    const placeTrades = useCallback(
        async (
            ...
            addLog(
                `[Round ${totalRoundsRef.current + 1}] Requesting proposals for ${allConfigs.length} strategy variations on ${_market}…`,
                'info'
            );
            ...
            <div className='multi-trader__content'>
                ...
                <div className='multi-trader__card multi-trader__log-card'>
                    ...
                    >
                        ...
                        <span className='multi-trader__expand-hint'>
                        ...
                        <div className='multi-trader__log-container'>
                            <div className='multi-trader__log-output'>
                                {logs.map(entry => (
                                    <div
                                        key={entry.id}
                                        className={`multi-trader__log-entry multi-trader__log-entry--${entry.type}`}
                                    >
                    ...
                    <div className='multi-trader__transactions-list'>
                        ...
                        <div className='multi-trader__transactions-scroll'>
                            ...
                            {transactions.length === 0 && (
                                <div className='multi-trader__empty'>No recent transactions</div>
                            )}
                        </div>
                    </div>
                </div>
