...
export const WalletsManager: React.FC<WalletsManagerProps> = ({ currency, activeLoginid }) => {
    ...
    const loadWalletsData = useCallback(async () => {
        ...
        try {
            ...
            if (defaultWallet) {
                const txRes = await DerivAccountWalletService.getWalletTransactions(
                    defaultWallet.wallet_type || 'fiat',
                    { limit: 20 }
                );
    ...
    const handleSimulateTransfer = () => {
        ...
        setTimeout(() => {
            setTransferStatus(
                `Successfully transferred ${transferAmount} ${currency || 'USD'} to ${transferTarget.toUpperCase()}!`
            );
            setTimeout(() => {
        ...
        <div className='deriv-wallets-hub'>
                    ...
                    <div className='dw-balance-group'>
                        ...
                        <div className='dw-balance-group__amount-row'>
                            <h2 className='dw-balance-group__value'>
                                {hideBalance
                                    ? '••••••••'
                                    : `${formatMoney(currency, totalEstValue || 0, true)} ${currency}`}
                            </h2>
                            <button
                    ...
                    {wallets.length === 0 ? (
                        ...
                        wallets.map(w => (
                            ...
                            >
                                ...
                                <div className='dw-item-row__right'>
                                    <span className='dw-item-row__balance'>
                                        {displayBalance(w.balance || 0, w.currency)}
                                    </span>
                                </div>
                            </div>
                        ))
            ...
            <div className='dw-section'>
                ...
                <div className='dw-trading-grid'>
                    ...
                    <div className='dw-item-row dw-item-row--card'>
                        ...
                        <div className='dw-item-row__right'>
                            <span className='dw-item-row__balance'>
                                {displayBalance(selectedWallet?.balance || 0, currency)}
                            </span>
                        </div>
                    </div>
            ...
            <div className='dw-transactions-action-box'>
                ...
                >
                    ...
                    <span>
                        {showTransactionsLedger ? localize('Hide transactions') : localize('View all transactions')}
                    </span>
                </button>
            </div>
                            ...
                            <tbody>
                                {filteredTransactions.length === 0 ? (
                                    <tr>
                                        <td
                                            colSpan={5}
                                            style={{ textAlign: 'center', padding: '3.6rem', color: '#858b97' }}
                                        >
                                    ...
                                    filteredTransactions.map(tx => (
                                        <tr key={tx.transaction_id}>
                                            <td>
                                                <span
                                                    className={`reports-action-badge reports-action-badge--${tx.action_type.toLowerCase()}`}
                                                >
                                            ...
                                            <td
                                                className={`reports-amount-text ${Number(tx.amount) >= 0 ? 'reports-amount-text--credit' : 'reports-amount-text--debit'}`}
                                            >
                                                {Number(tx.amount) >= 0
                                                    ? `+${formatMoney(currency, tx.amount, true)}`
                                                    : formatMoney(currency, tx.amount, true)}{' '}
                                                {currency}
                                            </td>
                    ...
                    <div className='wm-modal' onClick={e => e.stopPropagation()}>
                        ...
                        <div className='wm-modal__field'>
                            ...
                            <select value={transferSource} onChange={e => setTransferSource(e.target.value)}>
                                <option value='wallet'>
                                    US Dollar Wallet ({formatMoney(currency, selectedWallet?.balance || 0, true)}{' '}
                                    {currency})
                                </option>
                            </select>
                        </div>
                        ...
                        {transferStatus && <div className='wm-modal__status-msg'>{transferStatus}</div>}
