        ...
        <div className='account-management'>
                ...
                <div className='am-profile-grid'>
                    ...
                    <div className='am-card'>
                        ...
                        <div className='am-info-list'>
                            ...
                            <div className='am-info-row'>
                                ...
                                <span className='am-info-row__val'>
                                    {settings?.country || settings?.country_code || '—'}
                                </span>
                            </div>
                            <div className='am-info-row'>
                    ...
                    <div className='am-card'>
                        <div className='am-card__header'>
                            ...
                            <span
                                className={`pa-tag ${status?.status?.includes('authenticated') || isVirtual ? 'pa-tag--success' : 'pa-tag--warning'}`}
                            >
                                {status?.status?.includes('authenticated') || isVirtual
                                    ? localize('VERIFIED')
                                    : localize('PENDING')}
                            </span>
                        </div>
                        <div className='am-kyc-grid'>
                            <div className='am-kyc-tile'>
                                ...
                                <span
                                    className={`am-kyc-tile__status ${status?.authentication?.identity?.status === 'verified' || isVirtual ? 'am-kyc-tile__status--verified' : 'am-kyc-tile__status--pending'}`}
                                >
                                    {status?.authentication?.identity?.status === 'verified' || isVirtual
                                        ? '● Verified'
                                        : '○ Pending'}
                                </span>
                            </div>
                            <div className='am-kyc-tile'>
                                ...
                                <span
                                    className={`am-kyc-tile__status ${status?.authentication?.document?.status === 'verified' || isVirtual ? 'am-kyc-tile__status--verified' : 'am-kyc-tile__status--pending'}`}
                                >
                                    {status?.authentication?.document?.status === 'verified' || isVirtual
                                        ? '● Verified'
                                        : '○ Pending'}
                                </span>
                            </div>
                            ...
                            <div className='am-kyc-tile'>
                                ...
                                <span className='am-kyc-tile__status am-kyc-tile__status--verified'>● OAuth 2.0</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {activeSection === 'linked' && (
                <div className='am-card'>
                    <div className='am-card__header'>
                        ...
                        <span className='pa-tag pa-tag--muted'>
                            {linkedAccounts.length} {localize('Accounts')}
                        </span>
                    </div>
                    <div className='am-linked-list'>
                        {linkedAccounts.map(acc => (
                            <div
                                key={acc.id}
                                className={`am-linked-item ${acc.isActive ? 'am-linked-item--active' : ''}`}
                            >
                                <div>
                                    <span className='am-linked-item__id'>{acc.id}</span>
                                    <div className='am-linked-item__type'>
                                        {acc.isDemo
                                            ? localize('Demo Account • Virtual Currency')
                                            : localize('Real Account • Fiat Currency')}
                                    </div>
                                </div>
                                <div>
                                    {acc.isActive ? (
            ...
            {activeSection === 'markup' && (
                <div className='am-card'>
                    ...
                    {markupStats &&
                    (markupStats.total_turnover > 0 || (markupStats.breakdown && markupStats.breakdown.length > 0)) ? (
                        <div className='am-markup-grid'>
                            <div className='am-markup-card'>
                                ...
                                <h3 className='am-markup-card__val'>
                                    ${formatMoney('USD', markupStats?.total_turnover || 0, true)}
                                </h3>
                            </div>
                            <div className='am-markup-card'>
                                <span className='am-markup-card__label'>{localize('Total Commission')}</span>
                                <h3 className='am-markup-card__val am-markup-card__val--green'>
                                    ${formatMoney('USD', markupStats?.total_markup || 0, true)}
                                </h3>
                            </div>
                            <div className='am-markup-card'>
                                <span className='am-markup-card__label'>{localize('Transactions')}</span>
                                <h3 className='am-markup-card__val'>
                                    {addComma(markupStats?.total_transactions || 0)}
                                </h3>
                            </div>
                        </div>
                    ) : (
                        <div className='pa-empty-state'>
                            ...
                            <p className='pa-empty-state__description'>
                                {localize(
                                    'Developer markup commissions will appear here when registered as an official Deriv App partner.'
                                )}
                            </p>
                        </div>
                    )}
