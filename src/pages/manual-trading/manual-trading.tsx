...
const ManualTrading = observer(() => {
    ...
    const [isParallelRuns, setIsParallelRuns] = useState(false);
    ...
    const handleManualPurchase = useCallback(
        async (action: TManualTradeAction) => {
            ...
            try {
                ...
                if (isParallelRuns && runCount > 1) {
                    setTradeMessage(`Purchasing ${runCount} ${action.label} contracts in parallel...`);
                    const runPromises = Array.from({ length: runCount }).map(async (_, i) => {
                        const runIndex = i + 1;
                        const effectiveStake = getMartingaleStakeForRun({
                            stake,
                            currentLossStreak: 0, // Martingale is sequential, parallel runs all use base stake
                        ...
                        const verificationId = `manual_${selectedSymbol}_${tradeStartTime}_${runIndex}_${Math.random().toString(36).slice(2, 11)}`;
                    ...
                    totalProfit = results.reduce((acc, profit) => acc + profit, 0);
                    // Determine streak based on the last completed contract or overall? Overall logic:
                    activeLossStreak = totalProfit < 0 ? activeLossStreak + 1 : 0;
                ...
                <div className='manual-trading-ticket__inputs'>
                    ...
                    <label className='manual-trading-field manual-trading-field--checkbox' style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <input
                            ...
                            onChange={e => setIsParallelRuns(e.target.checked)}
