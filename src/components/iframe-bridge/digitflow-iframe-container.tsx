...
export const DigitFlowIframeContainer: React.FC<DigitFlowIframeContainerProps> = observer(
    ({ standaloneUrl, className = '', onLoad, hideHeader = true }) => {
        ...
        useEffect(() => {
            ...
            const fetchAuth = async () => {
                const loginid =
                    V2GetActiveClientId() || client?.loginid || localStorage.getItem('active_loginid') || '';
        ...
        return (
            <div
                className={`digitflow-standalone-container ${className}`}
